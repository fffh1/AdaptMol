...
from adaptmol.dataset import TrainDataset, AuxTrainDataset, bms_collate
from adaptmol.model import Encoder, Decoder
from adaptmol.loss import Criterion
from adaptmol.utils import seed_torch, save_args, init_summary_writer, LossMeter, AverageMeter, asMinutes, timeSince, \
    print_rank_0, format_df
from adaptmol.chemistry import convert_graph_to_smiles, keep_main_molecule
from adaptmol.tokenizer import get_tokenizer
...
def get_args():
    ...
    parser.add_argument('--do_coor', action='store_true')
...
def get_model(args, tokenizer, device, load_path=None):
    ...
    if args.local_rank != -1:
        encoder = DDP(encoder, device_ids=[args.local_rank], output_device=args.local_rank,static_graph=True)#,find_unused_parameters=True)
        decoder = DDP(decoder, device_ids=[args.local_rank], output_device=args.local_rank,static_graph=True)#,find_unused_parameters=True)
...
@torch.no_grad()
def validation_fn(valid_loader, encoder, decoder, criterion, device, args,epoch):#
    ...
    for step, (indices, images, refs, heatmap) in enumerate(valid_loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            ...
            losses = criterion(results, refs, heatmap)
            loss = sum(losses.values())
        if step < 1 and args.local_rank ==0:
            predictions, targets, _, seq_heatmap,_ = results['chartok_coords']
        ...
        if step % args.print_freq == 0 or step == (len(valid_loader) - 1):
            ...
            print_rank_0('Valid: [{0}/{1}] Data {data_time.avg:.3f}s Loss: {loss.avg:.4f} ({loss_str})'
                         .format(step, len(valid_loader), data_time=data_time, loss=loss_meter, loss_str=loss_str))

        if total_samples >= max_samples:
            break

    mean_losses = {k: v.avg for k, v in loss_meter.subs.items()}
    return mean_losses

def train_fn(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch,
             encoder_scheduler, decoder_scheduler, scaler, device, global_step, SUMMARY, args, mmd_loader = None):
    ...
    encoder.train()
    decoder.train()
    gt_iter = iter(train_loader)
    mmd_iter = None
    if mmd_loader is not None:
        mmd_iter = iter(mmd_loader)
    start = end = time.time()
    encoder_grad_norm = decoder_grad_norm = 0
    
    for step, (indices, images, refs, heatmap) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            ...
            if epoch >= 0:
                ...
                losses["gt_heatmap"] = 0.5 * losses["gt_heatmap"]
            
            loss = sum(losses.values())
        if mmd_loader is not None:
            try:
               
                mmd_indices, mmd_images, mmd_refs, mmd_heatmap = next(mmd_iter)
            ...
            if mmd_iter is not None:  
                mmd_images = mmd_images.to(device)          
                with torch.cuda.amp.autocast(enabled=args.fp16):
                
                    mmd_features, mmd_hiddens = encoder(mmd_images, mmd_refs)
                    mmd_results = decoder(mmd_features, mmd_hiddens, mmd_refs, mmd=True)
                    
                    mmd_losses = criterion(results, refs, heatmap, mmd_results)
                    if mmd_losses is not None:

                        mmd_loss = sum(mmd_losses.values())
                        
                       
                        loss = loss + mmd_loss
                        
                        
                        for key, value in mmd_losses.items():
                            losses[key] = value
                    else:
                        print_rank_0("mmd")        
        # record loss
...
def train_loop(args, train_df, valid_df, aux_df, tokenizer, save_path, our_validation,mmd_df, test_df, finetune_df, finetune_df1):
    SUMMARY = None
    ...
    if finetune_df is not None:
        print(len(finetune_df),"first self training df ")
        finetune_dataset = TrainDataset(args, finetune_df, tokenizer, split='train', dynamic_indigo=False, psudo_label = True)
    if finetune_df1 is not None:
        print(len(finetune_df1),"second slef training df")
        finetune_dataset1 = TrainDataset(args, finetune_df1, tokenizer, split='train', dynamic_indigo=False, psudo_label = True)


    if mmd_df is not None:
        print(len(mmd_df),"mmd_df for mmd")
    if aux_df is None and mmd_df is not None:
        train_dataset = TrainDataset(args, train_df, tokenizer, split='train', dynamic_indigo=False)
        val_dataset = TrainDataset(args, our_validation, tokenizer, split='train', dynamic_indigo=False)
        mmd_dataset = TrainDataset(args, mmd_df, tokenizer, split='test', dynamic_indigo=False)
    elif aux_df is None:
        train_dataset = TrainDataset(args, train_df, tokenizer, split='train', dynamic_indigo=False)
        val_dataset = TrainDataset(args, our_validation, tokenizer, split='train', dynamic_indigo=False)
    elif aux_df is not None and mmd_df is None :
        ...
        mmd_dataset = TrainDataset(args, mmd_df, tokenizer, split='test', dynamic_indigo=False)
    repeat_times = 1
    second_repeat_times = 1
    if finetune_df is not None and finetune_df1 is not None:
        repeated_finetune = [finetune_dataset] * repeat_times
        repeated_finetune1 = [finetune_dataset1] * second_repeat_times
        combined_train_dataset = ConcatDataset([train_dataset] + repeated_finetune + repeated_finetune1 )
    elif finetune_df is not None:
        repeated_finetune = [finetune_dataset] * repeat_times
        combined_train_dataset = ConcatDataset([train_dataset] + repeated_finetune)
    elif finetune_df1 is not None:
        repeated_finetune1 = [finetune_dataset1] * second_repeat_times
        combined_train_dataset = ConcatDataset([train_dataset] + repeated_finetune1)
    else:
        combined_train_dataset = ConcatDataset([train_dataset])
    # combined_train_dataset = ConcatDataset([train_dataset, finetune_dataset]) if finetune_df is not None else train_dataset    
    if args.local_rank != -1:
        train_sampler = DistributedSampler(combined_train_dataset, shuffle=True)
        valid_sampler = DistributedSampler(val_dataset, shuffle=True)
        if mmd_df is not None:
            mmd_sampler = DistributedSampler(mmd_dataset, shuffle=True)
    else:
        train_sampler = RandomSampler(combined_train_dataset)
        valid_sampler = RandomSampler(val_dataset)
        if mmd_df is not None:
            mmd_sampler = RandomSampler(mmd_dataset)
    ...
    validation_loader = DataLoader(val_dataset,
                              ...
                              collate_fn=bms_collate)
    if mmd_df is not None:
        mmd_loader = DataLoader(mmd_dataset,
                              batch_size=args.batch_size,
                              sampler=mmd_sampler,
                              num_workers=args.num_workers,
                              prefetch_factor=4,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if mmd_df is None:
            ...
            avg_loss, global_step = train_fn(
                train_loader,encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch,
                encoder_scheduler, decoder_scheduler, scaler, device, global_step, SUMMARY, args, mmd_loader)
        
        # # validation 
        mean_loss = validation_fn(validation_loader, encoder,decoder,criterion,device,args,epoch)
        for k,v in mean_loss.items():
            print_rank_0(f'Epoch {epoch + 1} - {k}: {v:.4f}')
        # eval
        ...
        #     for df in test_df:
        #         scores1 = inference(args, df, tokenizer,encoder, decoder, save_path=save_path, split='test')
        #         print_rank_0(json.dumps(scores1, indent=4))
        ...
        for name in ["combined",'post_smiles', 'graph_smiles', 'canon_smiles']:
            if name in scores:
                score = scores[name]
                break
...
def build_mol_paths(data_path, image_names, dataset_type):
    ...
    dataset_dirs = {
        "USPTO.csv": "USPTO_mol_ref",
        "USPTO_p.csv": "USPTO_mol_ref",
        "JPO.csv": "JPO_mol_ref", 
        "JPO_p.csv": "JPO_mol_ref",
        "CLEF.csv": "CLEF_mol_ref",
        "CLEF_p.csv": "CLEF_mol_ref",
        "UOB.csv": "UOB_mol_ref",
        "UOB_p.csv": "UOB_mol_ref",
        "clean.csv": "USPTO-30K/clean/mols",
        "large.csv": "USPTO-30K/large/mols",
        "abbreviated.csv": "USPTO-30K/abbreviated/mols",
    }
    
    if dataset_type not in dataset_extensions:
        return None
        
    base_dir = os.path.join(data_path,"data",dataset_dirs[dataset_type])
    extension = dataset_extensions[dataset_type]
    
   
    mol_paths = []
    for image_name in image_names:
        mol_file = f"{image_name}{extension}"
        full_path = os.path.join(base_dir, mol_file)
...
def inference(args, data_df, tokenizer, encoder=None, decoder=None, save_path=None, split='test'):
    ...
    dataset = TrainDataset(args, data_df, tokenizer, split=split, pad =pad)
    if args.local_rank != -1:
    ...
    if 'image_id' not in data_df.columns or "JPO" in data_df.attrs['file'].split('/')[-1] or "UOB_p" in data_df.attrs['file'].split('/')[-1]  or "USPTO_p" in data_df.attrs['file'].split('/')[-1]   or "CLEF_p" in data_df.attrs['file'].split('/')[-1]:
        data_df['image_id'] = [path.split('/')[-1].split('.')[0] for path in data_df['file_path']]
    
    pred_df = data_df[['image_id', 'file_path']].copy()
    ...
    if 'edges' in args.formats:
        ...
        smiles_list, molblock_list, r_success = convert_graph_to_smiles(
            pred_df['node_coords'], pred_df['node_symbols'], pred_df['edges'],abb=abbre)
    ...
    if args.keep_main_molecule:
        print("keep main molecule")
        pred_df['graph_SMILES'] = keep_main_molecule(pred_df['graph_SMILES'])
        
    
    # Compute scores
    if 'SMILES' in data_df.columns:
        print(data_df.attrs['file'].split('/')[-1])
        if "mol_path" in data_df.columns:
            mol_path = data_df["mol_path"]
            mol_path = [os.path.join(args.data_path,"data", path) for path in mol_path]
        else:
            mol_path = build_mol_paths(args.data_path, data_df["image_id"], data_df.attrs['file'].split('/')[-1])
        evaluator = SmilesEvaluator(data_df['SMILES'], tanimoto=True, mol_path = mol_path )
        ...
        if 'graph_SMILES' in pred_df.columns:
            ...
            # scores["molfile"] = graph_scores['molfile_smiles']
            scores["combined"] = graph_scores["combined"]
            scores["inchi"] = graph_scores["inchi"]
            for i in range(len(graph_scores["canon_smiles_details"])):
                if "combined_detail" in graph_scores:
                    if graph_scores["combined_detail"][i] == False:
                        wrong_case.add(data_df["image_id"][i])
                    else:
                        correct_case.add(data_df["image_id"][i])
                else:
                    if graph_scores["canon_smiles_details"][i] == True:
                        correct_case.add(data_df["image_id"][i])
                    else:
                        wrong_case.add(data_df["image_id"][i])
            print("===============")
            if "molfile" in graph_scores:
                scores['graph_molfile'] = graph_scores["molfile"]
                scores["graph_mofile_inchi"] = graph_scores["mofile_inchi"]
...
def get_chemdraw_data(args):
    ...
    if args.do_train:
        ...
        train_df = pd.concat([pd.read_csv(os.path.join(args.data_path, file)) for file in train_files], ignore_index=True)
        ...
        if args.validation_file:
            validation_df = pd.read_csv(os.path.join(args.data_path, args.validation_file))
        if args.mmd_file:
            mmd_df = pd.read_csv(os.path.join(args.data_path, args.mmd_file))
            print_rank_0(f'mmd_df.shape: {mmd_df.shape}')
...
def read_pesudo_lable(json_file, csv_file):
    ...
    first_file = csvfiles[0]
    first_index = index[0]
    with open(first_index, 'r', encoding='utf-8') as f:
        data = json.load(f)
    # ??CSV
    df = pd.read_csv(first_file)
    # ????
    ids = data['correct']
    filtered_df = df[df['image_id'].astype(str).isin(ids)]
    
    filtered_df = filtered_df[["node_symbols", "node_coords", "edges","file_path"]]
    filtered_df = filtered_df.reset_index(drop=True)
    
    second_file = csvfiles[1]
    second_index = index[1]
    with open(second_index, 'r', encoding='utf-8') as f:
        data1 = json.load(f)
    # ??CSV
    df1 = pd.read_csv(second_file)
    # ????
    ids = data1['correct']
    filtered_df1 = df1[df1['image_id'].astype(str).isin(ids)]
    filtered_df1 = filtered_df1[["node_symbols", "node_coords", "edges","file_path"]]
    filtered_df1 = filtered_df1.reset_index(drop=True)



    return filtered_df, filtered_df1
...
def main():
    ...
    train_df, valid_df, test_df, aux_df, tokenizer, validation_df ,mmd_df = get_chemdraw_data(args)

    if args.do_train:
        train_loop(args, train_df, valid_df, aux_df, tokenizer, args.save_path, validation_df,mmd_df,test_df, finetune_df,finetune_df1)
